...
fn record_default_value_test() {
    ...
    let mir_view =
        StdfRecordView::read_from_bytes(rec_header, &empty_raw_data, &ByteOrder::LittleEndian);
    if let StdfRecord::MIR(ref inner) = mir_rec {
        ...
        if let StdfRecordView::MIR(v) = mir_view {
            assert_eq!(
                v.mode_cod(),
                inner.mode_cod,
                "Testing default value of MIRView and MIR"
            );
            assert_eq!(
                v.rtst_cod(),
                inner.rtst_cod,
    ...
    if let StdfRecord::MRR(ref inner) = mrr_rec {
        ...
        if let StdfRecordView::MRR(v) = mrr_view {
            assert_eq!(
                v.disp_cod(),
                inner.disp_cod,
                "Testing default value of MRRView and MRR"
            );
        }
    }
    ...
    if let StdfRecord::PCR(ref inner) = pcr_rec {
        ...
        if let StdfRecordView::PCR(v) = pcr_view {
            assert_eq!(
                v.rtst_cnt(),
                inner.rtst_cnt,
                "Testing default value of PCRView and PCR"
            );
            assert_eq!(
                v.abrt_cnt(),
                inner.abrt_cnt,
    ...
    if let StdfRecord::HBR(ref inner) = hbr_rec {
        ...
        if let StdfRecordView::HBR(v) = hbr_view {
            assert_eq!(
                v.hbin_pf(),
                inner.hbin_pf,
                "Testing default value of HBRView and HBR"
            );
        }
    }
    ...
    if let StdfRecord::SBR(ref inner) = sbr_rec {
        ...
        if let StdfRecordView::SBR(v) = sbr_view {
            assert_eq!(
                v.sbin_pf(),
                inner.sbin_pf,
                "Testing default value of SBRView and SBR"
            );
        }
    }
    ...
    if let StdfRecord::PMR(ref inner) = pmr_rec {
        ...
        if let StdfRecordView::PMR(v) = pmr_view {
            assert_eq!(
                v.chan_typ(),
                inner.chan_typ,
                "Testing default value of PMRView and PMR"
            );
            assert_eq!(
                v.head_num(),
                inner.head_num,
    ...
    if let StdfRecord::CDR(ref inner) = cdr_rec {
        ...
        if let StdfRecordView::CDR(v) = cdr_view {
            assert_eq!(
                v.inv_val(),
                inner.inv_val,
                "Testing default value of CDRView and CDR"
            );
        }
    }
    ...
    if let StdfRecord::WIR(ref inner) = wir_rec {
        ...
        if let StdfRecordView::WIR(v) = wir_view {
            assert_eq!(
                v.site_grp(),
                inner.site_grp,
                "Testing default value of WIRView and WIR"
            );
        }
    }
    ...
    if let StdfRecord::WRR(ref inner) = wrr_rec {
        ...
        if let StdfRecordView::WRR(v) = wrr_view {
            assert_eq!(
                v.site_grp(),
                inner.site_grp,
                "Testing default value of WRRView and WRR"
            );
            assert_eq!(
                v.rtst_cnt(),
                inner.rtst_cnt,
    ...
    if let StdfRecord::WCR(ref inner) = wcr_rec {
        ...
        if let StdfRecordView::WCR(v) = wcr_view {
            assert_eq!(
                v.wafr_siz(),
                inner.wafr_siz,
                "Testing default value of WCRView and WCR"
            );
            assert_eq!(
                v.die_ht(),
                inner.die_ht,
    ...
    if let StdfRecord::PRR(ref inner) = prr_rec {
        ...
        if let StdfRecordView::PRR(v) = prr_view {
            assert_eq!(
                v.soft_bin(),
                inner.soft_bin,
                "Testing default value of PRRView and PRR"
            );
            assert_eq!(
                v.x_coord(),
                inner.x_coord,
    ...
    if let StdfRecord::TSR(ref inner) = tsr_rec {
        ...
        if let StdfRecordView::TSR(v) = tsr_view {
            assert_eq!(
                v.test_typ(),
                inner.test_typ,
                "Testing default value of TSRView and TSR"
            );
            assert_eq!(
                v.exec_cnt(),
                inner.exec_cnt,
    ...
    if let StdfRecord::FTR(ref inner) = ftr_rec {
        ...
        if let StdfRecordView::FTR(v) = ftr_view {
            assert_eq!(
                v.patg_num(),
                inner.patg_num,
                "Testing default value of FTRView and FTR"
            );
        }
    }
}
