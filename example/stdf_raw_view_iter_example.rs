...
fn main() {
    ...
    //
    // unlike `stdf_raw_iter_example.rs`, the view iterator reuses one buffer and
    // yields a borrowed `RawDataElementView`, so it cannot be a std `Iterator`;
    // drive it with an inherent `next` in a `while let` loop instead.
    let mut iter = reader.get_rawdata_view_iter();
    while let Some(item) = iter.next() {
        let raw = item.unwrap();
        if !raw.is_type(rec_types) {
            continue;
        }
